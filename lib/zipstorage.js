...
var ZipStorageCore = require('./zipstorage-core');
var TaskQueue = require('./taskqueue');
var d64 = require('d64');

function ZipStorage(dbname) {
  this._store = new ZipStorageCore(dbname);
  this._queue = new TaskQueue();
}

ZipStorage.prototype.sequentialize = function (callback, fun) {
  this._queue.add(fun, callback);
};

ZipStorage.prototype.init = function (callback) {
  var self = this;
...
ZipStorage.prototype.keys = function (callback) {
  var self = this;
  self.sequentialize(callback, function (callback) {
    self._store.getKeys(function (err, keys) {
...
ZipStorage.prototype.setItem = function (key, value, callback) {
  var self = this;
  self.sequentialize(callback, function (callback) {
...
ZipStorage.prototype.getItem = function (key, callback) {
  var self = this;
  self.sequentialize(callback, function (callback) {
...
ZipStorage.prototype.removeItem = function (key, callback) {
  var self = this;
  self.sequentialize(callback, function (callback) {
    var idx = utils.sortedIndexOf(self._keys, key);
    if (self._keys[idx] === key) {
...
ZipStorage.prototype.length = function (callback) {
  var self = this;
  self.sequentialize(callback, function (callback) {
