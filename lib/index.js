...
class ZipDOWN {
  ...
  async open() {
    ...
    this._rootPath = file;
    console.log("OPEN END",fs.existsSync(this.location), this._rootPath, this.location,this.ext, Object.keys(ZipDOWN.openFile), Object.keys(ZipDOWN.writeFile));
    if (CACHE[this.location]) {
      this._store = CACHE[this.location];
    } else {
      this._store = new Zip();
      if (fs.existsSync(this.location)) {
        let buffer = await (ZipDOWN.openFile[this.ext] || ZipDOWN.openFile[ALL_EXT] || ((location) => fs.readFileSync(location)))(this.location);
