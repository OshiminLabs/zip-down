...
class ZipDOWN {
   ...
   */
  _setName(k) {
    return this._rootPath + k;
  }
  _writeZipFile() {
    clearTimeout(this._ttl);
    this._ttl = setTimeout(async () => {
      let buffer = await this._store.generateAsync({
        type: "nodebuffer"
      });
      (
  ...
  async get(k) {
    const v = this._store.file(this._setName(k))
    if (v === null)
